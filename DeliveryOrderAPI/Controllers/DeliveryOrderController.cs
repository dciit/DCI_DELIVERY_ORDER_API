...
namespace DeliveryOrderAPI.Controllers
{
    ...
    public class DeliveryOrderController : Controller
    {
        ...
        private Services serv = new();
        //double StockHistory = 0;
        ...
        //***********************************************************************//
        //                          END -- NEW VERSION USE MODEL ONLY                   //
        //***********************************************************************//
        ...
        //[Authorize]
        [Route("/getPlans")]
        public async Task<IActionResult> getPlans([FromBody] MGetPlan obj)
        {
            MGetPlanDataSet plans = GetPlanDataSet(obj.vdCode, obj.buyer, false); // CalDoWhenStockMinus = มาจากการกดปุ่ม RUN D/O หรือไม่ ?
            //MGetPlanDataSet res = new MGetPlanDataSet();
            //MRESRESULT res = getPlan();
            //return Ok(res);
            return Ok(plans);
        }
...
AND VENDER = @VD_CODE  GROUP BY PRDYMD,MODEL,PARTNO,CM,VENDER ORDER BY PRDYMD ASC";
        ...
        {
            SqlCommand sql = new SqlCommand();
            sql.CommandText = @"SELECT DISTINCT DICT.CODE,CONCAT(EMP.NAME,' ',EMP.SURN) AS NAME FROM [dbSCM].[dbo].[DO_DictMstr] DICT LEFT JOIN  [dbHRM].[dbo].[Employee] EMP 
ON DICT.CODE  COLLATE SQL_Latin1_General_CP1_CI_AS = EMP.CODE  COLLATE SQL_Latin1_General_CP1_CI_AS  WHERE DICT.DICT_TYPE = 'BUYER'";
            DataTable dt = dbSCM.Query(sql);
            return Ok(ConvertDataTableasJSON(dt));
        }
        ...
        [HttpGet]
        [Route("/vender/{vdcode}")]
        public IActionResult GetVenderDetail(string vdcode)
        {
            SqlCommand sql = new SqlCommand();
            sql.CommandText = @"SELECT TOP(1) A.*,B.REF_CODE,B.NOTE,B.DICT_ID FROM [dbSCM].[dbo].[DO_VENDER_MASTER] A LEFT JOIN [dbSCM].[dbo].[DO_DictMstr] B ON A.VD_CODE = B.CODE WHERE A.VD_CODE = @VDCODE";
            sql.Parameters.Add(new SqlParameter("@VDCODE", vdcode));
            DataTable dt = dbSCM.Query(sql);
            return Ok(ConvertDataTableasJSON(dt));
        }
        ...
        {
            //var content = _contextDBSCM.DoDictMstrs.FirstOrDefault(x => x.DictType == "VENDER_BOX_CAPACITY" && x.Code == param.vender);
            //if (content != null)
            //{
            //    content.RefCode = param.min.ToString();
            //    content.Note = param.max.ToString();
            //}
...
WHERE BoardId = 303 AND InsertDate >= '2023-09-07 19:08:00' AND Insertdate <= '2023-09-07 19:20:00' ORDER BY InsertDate ASC";
        ...
        public IActionResult getJWT([FromBody] MLogin param)
        {
            return Ok(new
            {
                jwt = CreateToken(param.username)
            });
        }
