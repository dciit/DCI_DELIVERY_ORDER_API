...
namespace DeliveryOrderAPI
{
    public class Services
    {
        ...
        public List<MDOAct> GetDoActs(DateTime sDate, DateTime eDate, string Parts, string SupplierCode)
        {
            ...
            OracleCommand cmd = new();
            //cmd.CommandText = "SELECT * FROM DST_DATAC1 WHERE TRIM(PARTNO) IN ('" + Parts + "') AND TRIM(ACDATE) BETWEEN '" + sDate.ToString("yyyyMM") + "' AND '" + eDate.ToString("yyyyMM") + "' ";
            cmd.CommandText = "SELECT PARTNO,SUM(WQTY) AS WQTY,ACDATE FROM DST_DATAC1 WHERE TRIM(PARTNO) IN ('" + Parts + "') AND TRIM(ACDATE) BETWEEN '" + sDate.ToString("yyyyMM") + "' AND '" + eDate.ToString("yyyyMM") + "' AND HTCODE = '" + SupplierCode + "' GROUP BY PARTNO,ACDATE";
            //cmd.CommandText = "SELECT PARTNO,SUM(WQTY) AS WQTY,ACDATE FROM DST_DATAC1 WHERE TRIM(PARTNO) IN ('" + Parts + "') AND TRIM(ACDATE) BETWEEN '" + sDate.ToString("yyyyMM") + "' AND '" + eDate.ToString("yyyyMM") + "' GROUP BY PARTNO,ACDATE";
            DataTable dt = dbAlpha2.Query(cmd);
            foreach (DataRow dr in dt.Rows)
            {
        ...
        internal List<MPO> GetPoDIT(string vdCode, DateTime sDate, DateTime eDate,string PartJoin)
        {
            List<MPO> res = new List<MPO>();
            OracleCommand cmd = new();
            cmd.CommandText = @"SELECT A.PARTNO,A.DELYMD,SUM(A.WHBLBQTY) AS WHBLBQTY FROM GST_DATOSD A LEFT JOIN GST_DATOSC B ON A.PONO = B.PONO WHERE  A.apbit in ('U','P') AND TRIM(A.PARTNO) IN ('" + PartJoin + "') AND B.HTCODE = '" + vdCode + "' AND A.DELYMD >= '" + sDate.ToString("yyyyMMdd") + "' AND A.DELYMD <= '" + eDate.ToString("yyyyMMdd") + "' GROUP BY A.PARTNO,A.DELYMD";
